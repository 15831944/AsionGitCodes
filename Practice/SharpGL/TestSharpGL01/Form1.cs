...
namespace TestSharpGl_01
{
    public partial class Form1 : Form
    {
        ...
        private void gl1_OpenGLDraw(object sender, RenderEventArgs args)
        {
            ...
            gl.Translate(-1.5f, 0.0f, -6.0f);          // 左移 1.5 單位，並移入屏幕 6.0
            gl.Rotate(rtri, 0.0, 1.0, 0.0);
	
            gl.Begin(OpenGL.GL_TRIANGLES);  // 繪製三角形
            gl.Color(1.0f, 0.0f, 0.0f);     // 設置當前色為紅色
            gl.Vertex(0.0f, 1.0f, 0.0f);    // 上頂點
            gl.Color(0.0f, 1.0f, 0.0f);		// 設置當前色為綠色
            gl.Vertex(-1.0f, -1.0f, 0.0f);  // 左下
            gl.Color(0.0f, 0.0f, 1.0f);		// 設置當前色為藍色
            gl.Vertex(1.0f, -1.0f, 0.0f);   // 右下
            gl.End();   // 三角形繪製結束

            gl.LoadIdentity();					// 重置模型觀察矩陣
            gl.Translate(1.5, 0.0, -6.0);		// 右移1.5單位,並移入屏幕 6.0
            gl.Rotate(rquad, 1.0, 0.0, 0.0);	//  繞X軸旋轉四邊形

            gl.Color(0.5f, 0.5f, 1.0f);	    // 一次性將當前色設置為藍色
            gl.Begin(OpenGL.GL_QUADS);      //  繪製正方形
            gl.Vertex(-1.0f, 1.0f, 0.0f);   // 左上
            gl.Vertex(1.0f, 1.0f, 0.0f);    // 右上
            gl.Vertex(1.0f, -1.0f, 0.0f);   // 左下
            gl.Vertex(-1.0f, -1.0f, 0.0f);  // 右下
            gl.End();
