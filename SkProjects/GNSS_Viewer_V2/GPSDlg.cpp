...
{
	if(CGPSDlg::gpsDlg->m_isNmeaFileOpen)
	{			
		char BINMSG[100] = {0};
		buffer[offset] = 13;
		buffer[offset+1] = 10;
		...
		CGPSDlg::gpsDlg->m_saveNmea->DisplaySize(CGPSDlg::gpsDlg->m_nmeaFileSize);
	}
...
void CGPSDlg::Copy_NMEA_Memery()
{
	...
	if(m_bdgsaMsgCopy1.SatelliteID[0]==0)
	{
		int a = 0;
	}
	...
	if(m_bdgsvMsgCopy1.NumOfSate==0)
	{
		int a = 0;
	}
	...
	if(copygsv || copygsv_gnss || copygsv_bd)
	{
		CopyNmeaToUse();
...
bool CGPSDlg::NmeaProc(const char* buffer, int offset)
{	
	if(m_isNmeaFileOpen && NMEA_INPUT)
	{	//NMEA Player version
		m_nmeaFileSize += offset + 1;
		m_saveNmea->DisplaySize(m_nmeaFileSize);
	}

	BOOL isNmea = FALSE;
	for(int i=offset-1; i>0; --i)
	{	//Find the '*' char position from tail.
		if(buffer[i] == '*' )
		{
			offset = i + 3;
			isNmea = TRUE;
	...
	NmeaType msgType = NMEA::MessageType(buffer, offset);	
	NmeaOutput(buffer, offset);
	
	if(msgType==MSG_REBOOT)
	{
#if _TEST_KERNEL_
		++m_rebootCount;
		if(++m_rebootCount > 2)
		{
			KillTimer(TEST_KERNEL_TIMER);
			PostMessage(UWM_KERNEL_REBOOT);
		}
#else
		PostMessage(UWM_FIRST_NMEA, 1000);
#endif
	}

	if(m_isNmeaFileOpen)
	{	//Save NMEA is enable.			
		m_nmeaFile.Write(buffer, offset + ((isNmea) ? 2 : 1));
		m_nmeaFileSize += offset;
		m_saveNmea->DisplaySize(m_nmeaFileSize);
	}
	...
	case MSG_GLGSA:
		...
		break;
	case MSG_GNGSA:
		nmea.ShowGNGSAmsg(m_gpgsaMsgCopy, m_glgsaMsgCopy, m_bdgsaMsgCopy, buffer, offset);	
		break;
	case MSG_BDGSA:
		nmea.ShowBDGSAmsg(m_bdgsaMsgCopy, buffer, offset);	
		break;
	case MSG_GPGSA:		
		//nmea.ShowGPGSAmsg(m_gpgsaMsgCopy, buffer, offset);	
		nmea.ShowGNGSAmsg(m_gpgsaMsgCopy, m_glgsaMsgCopy, m_bdgsaMsgCopy, buffer, offset);	
		//for olinkstar and gg16a
		//nmea.ShowGPGSAmsg(m_gpgsaMsgCopy, m_glgsaMsgCopy, m_bdgsaMsgCopy, buffer, offset);	
		break;
	case MSG_GGA:		
...
//		ShowTime();
	...
	case MSG_ZDA:			
		...
		break; 		
	case MSG_GPGSV:		
		//nmea.ShowGPGSVmsg(m_gpgsvMsgCopy, buffer, offset);	
		//for olinkstar and gg16a
		nmea.ShowGPGSVmsg2(m_gpgsvMsgCopy, m_glgsvMsgCopy, m_bdgsvMsgCopy, buffer, offset);	
		break; 	
	case MSG_RMC:		
...
void CGPSDlg::BinaryProc(unsigned char* buffer,int len)
{
	if(!m_isConnectOn)
	{
		return;
	...
	unsigned char MSG_TYPE = Cal_Checksum(buffer);	
	//char message[512];
	...
	case 0xDC:		// measurement time
		...
		Show_Measurement_time(buffer);
		ShowTime();
		//NMEA_Output(message, strlen(message));
		break;
	case 0xDD:		// raw measurement
		Show_Measurement_channel(buffer);
		//Show_Message(buffer,len);
		break;
	case 0xDE:		// SV_CH status
		Show_Measurement_SV(buffer);
		//Show_Message(buffer,len);
		break;
	case 0xDF:		// receiver state
		Show_Receiver_nav(buffer);
		//Show_Message(buffer,len);
		break;
	case 0xE0:		// sub frame data
		//Show_Message(buffer,len);
		Show_subframe(buffer);
		break;
	case 0xE1:
		Show_glonass_string_buffer(buffer);
		break;
	case BINMSG_ECEF_USER_PVT:
		ShowBinaryOutput(buffer,len);
		ShowTime();
		break;
	}
...
UINT NMEA_Parsing_Thread(LPVOID pParam)
{
	...
	while(fgets(nmeaBuff, sizeof(nmeaBuff), f))
	{
		...
		if(CGPSDlg::gpsDlg->NmeaProc(nmeaBuff, strlen(nmeaBuff)))
		{		
...
//	DDX_Control(pDX, IDC_CLOCK_T, lbl_clock);	
	...
	DDX_Control(pDX, IDC_GNSS_BAR, *bdSnrBar);	
...
void CGPSDlg::CreateGPSThread()
{
	...
	TRACE("CreateGPSThread \r\n");
	//g_thread = (HANDLE)_beginthread(ConnectGPS, 0, msgWindow);
	g_gpsThread = ::AfxBeginThread(ConnectGPS, 0);
	//SetThreadPriority(thread,THREAD_PRIORITY_BELOW_NORMAL);
	//thread = AfxBeginThread(ConnectGPS, msgWindow, THREAD_PRIORITY_BELOW_NORMAL, 0, 0, NULL);
...
{	
	...
	else if(nIDEvent==TEST_KERNEL_TIMER)
	{
		...
		{
			KillTimer(nIDEvent);
			PostMessage(WM_COMMAND, MAKEWPARAM(IDC_DOWNLOAD, BN_CLICKED), NULL);
			//OnBnClickedDownload();
			return;
		}
		if(!m_setTtff)
		{
			SetKernelTestTTFF(m_ttffCount, 60 - m_testDuration);
		}
#endif
	}
...
{		
	...
	//GetDlgItem(IDC_CLOSE)->ShowWindow(0);	
...
/*
void CGPSDlg::error(LPSTR lpszFunction) 
{ 
	CHAR szBuf[80]; 
	DWORD dw = GetLastError(); 

	sprintf_s(szBuf, sizeof(szBuf), "%s failed: GetLastError returned %u\n", 
		lpszFunction, dw); 
...
{	//m_versionInfo IDC_SW_VER IDC_SW_REV m_swKernel m_swRev
	...
	{
		CString txt;
		txt.Format("%d.%d.%d", m_versionInfo[11], m_versionInfo[12], m_versionInfo[13]);
...
void CGPSDlg::ShowAltitude(void)
{
...
void CGPSDlg::ShowDirection(void)
{
...
//	if(m_isNmeaFileOpen)
//	{
//		m_saveNmea->OnBnClickedClose();	
//	}
//	if(m_playNmea != NULL)
//	{
//		m_playNmea->OnBnClickedClose();	
//	}	
...
{
	//	int n= AfxMessageBox("Would you like to Exit the SkyTraq GPS Utility?",MB_YESNOCANCEL|MB_ICONQUESTION   ,0);
	//	if(n==IDYES)
//	if(m_isNmeaFileOpen)
//	{
//		m_saveNmea->OnBnClickedClose();	
...
/*
void CGPSDlg::OnDownloadRomwrite()
{
...
void CGPSDlg::BaudrateError(void)
{
	AfxMessageBox("The buadrate setting is wrong!");
}

bool CGPSDlg::OpenUart()
{	
	return true;
}

void CGPSDlg::Nooutput()
{
	U08 msg[9];
	memset(msg, 0, 9);   		    
	msg[0]=(unsigned char)0xa0;
	msg[1]=(unsigned char)0xa1;
	msg[2]=0;
	msg[3]=2;
	msg[4]=9; //msgid
	msg[5]=0;
	msg[6]=9; //checksum right	    
	msg[7]=(unsigned char)0x0d;
	msg[8]=(unsigned char)0x0a;	
	SendToTarget_4800(msg, 9,"1111");		

}

void CGPSDlg::Nmeamessage()
{
	memset(m_inputMsg, 0, 9);   		    
	m_inputMsg[0]=(unsigned char)0xa0;
	m_inputMsg[1]=(unsigned char)0xa1;
	m_inputMsg[2]=0;
	m_inputMsg[3]=2;
	m_inputMsg[4]=9; //msgid
	m_inputMsg[5]=1;
	m_inputMsg[6]=8; //checksum right	    
	m_inputMsg[7]=(unsigned char)0x0d;
	m_inputMsg[8]=(unsigned char)0x0a;
	SendToTarget_4800(m_inputMsg, 9,"2222");	
}
*/
	...
	while(dwBytesRemaining)
	{
		...
		while(dwBytesRemaining)
		{
			...
			case FIXPOI2:
				...
				DataLog.V    = fix_full.word[0]     &0x7ff;				  
				...
				//tow = DataLog.TOW;
				//
				//DataLog.ECEF_X = (U32)buff[6]<<24;
				//DataLog.ECEF_X |= (U32)buff[7]<<16;
				//DataLog.ECEF_X |= (U32)buff[4]<<8;
				//DataLog.ECEF_X |= (U32)buff[5];
				...
				//memcpy(&DataLog.ECEF_X,&fix_full.word[3],4);
				//memcpy(&DataLog.ECEF_Y,&fix_full.word[5],4);
				//memcpy(&DataLog.ECEF_Z,&fix_full.word[7],4);
				DataLog.DECEF_X=0;
				DataLog.DECEF_Y=0;
				DataLog.DECEF_Z=0;
			...
			case FIXINC2:
				...
				DataLog.V        = fix_inc.word[0]       &0x7ff;
			...
			//xyz.px = DataLog.ECEF_X;
			//xyz.py = DataLog.ECEF_Y;
			//xyz.pz = DataLog.ECEF_Z;


			//cartesian_to_geodetic(&xyz,&lla);
			...
			// latitude : phi (rad.)
			double lat = atan2((Z+e2p*WGS84b*pow(sin(theta),3.0)),(p-e2*WGS84a*pow(cos(theta),3.0)));
			...
			double lon = atan2((double)Y , (double)X );

			double N = WGS84a/(sqrt(1-e*e*sin(lat)*sin(lat)));
			double h = p/cos(lat)-N;

			h = h - geo_id.GEO_calc_geoid_height(lat, lon);
			...
			else if(type == FIXFULL2 || type == FIXINC2)
			{
			...
			sprintf_s(Log, sizeof(Log), "%d %d %d:%d:%.0f %d/%d/%d %d %d %d %d %d %d %d %.6f %.6f %.6f %d\r\n",
				DataLog.WNO+1024,DataLog.TOW,utc.hour,utc.minute,utc.sec,utc.month,utc.day,utc.year,
				DataLog.DECEF_X,DataLog.DECEF_Y,DataLog.DECEF_Z,X,Y,Z,DataLog.V, lon,lat,h,type);
			FDecompress.Write(Log,(U16)strlen(Log)-1);
			dwBytesRemaining-=nBytesRead;	
			// ---------------------------------------------
			write_count++;
...
void CGPSDlg::close_nmea_file()
{
	...
	_save_nmea_cs.Unlock();
}

void CGPSDlg::stop_write_nmea()
{
	_save_nmea_cs.Lock();
	m_isNmeaFileOpen = false;	
	_save_nmea_cs.Unlock();
...
void CGPSDlg::continue_write_nmea()
{
	_save_nmea_cs.Lock();
...
void CGPSDlg::OnFileSaveNmea()
{
	...
	m_nmeaFile.Open(fileName, CFile::modeReadWrite | CFile::modeCreate | CFile::shareDenyWrite);
	m_isNmeaFileOpen = true;	
	m_isPressNmeaCommend = true;
	m_nmeaFilePath = m_nmeaFile.GetFilePath();
	m_nmeaFileSize = 0;
	m_saveNmea->ShowWindow(SW_SHOW);
	m_saveNmea->Initialize(m_nmeaFile.GetFileTitle());
	m_saveNmea->SetFocus();
...
void CGPSDlg::Load_Menu()
{
	...
	static MenuItemEntry menuItemBinary[] =
	{
		...
		{ !IS_DEBUG, MF_STRING, ID_QUERYSOFTWAREVERSION_SYSTEMCODE, "Query Software Version", NULL },
		{ !IS_DEBUG, MF_STRING, ID_QUERYSOFTWARECRC_SYSTEMCODE, "Query CRC Checksum", NULL },
//		{ BINARY_MESSAGE_SUPPORT, MF_STRING, ID_BINARY_QUERYBINARYMSGINTERVAL, "Query Binary Message Measurement", NULL },
		...
		{ (CUSTOMER_ID==0x0002), MF_STRING, ID_QUERY_ERICSSON_STC_ITV, "Query Ericsson Sentence Interval", NULL },
		...
		{ IS_DEBUG, MF_STRING, ID_QUERY1PPSTIMING_QUERY, "Query DOP Mask", NULL },//
...
//		{ AVID, MF_STRING, ID_BINARY_QUERYDISTANCEANGLE, "Query Distance && Angle", NULL },
		...
		{ (CUSTOMER_ID==0x0002), MF_STRING, ID_CONFIG_ERICSSON_STC_ITV, "Configure Ericsson Sentence Interval", NULL },
...
//		{ BINARY_MESSAGE_SUPPORT, MF_STRING, ID_BINARY_CONFIGUREBINARYMSGINTERVAL, "Configure Binary Message Measurement", NULL },
		...
		{ IS_DEBUG, MF_STRING, ID_CONFIGURE1PPSTIMING_CONFIGURE1PPS, "Configure DOP Mask", NULL },
...
//		{ INTERFERENCE, MF_STRING, ID_BINARY_CONFIGUREJAMMINGINTERFERENCE, "Configure Jamming Interference", NULL },
	...
	static MenuItemEntry menuItemVenus8[] =
	{
		...
		{ IS_DEBUG, MF_STRING, ID_BINARY_QUERY_PARAM_SEARCH_ENG_NUM, "Query Parameter Search Engine Number", NULL },
		...
		{ IS_DEBUG, MF_STRING, ID_CONFIGURE_PARAM_SEARCH_ENG_NUM, "Configure Parameter Search Engine Number", NULL },
...
void CGPSDlg::OnBnClickedDownload()
{
	...
	if(CUSTOMER_DOWNLOAD && CUSTOMER_ID==0x4F4C)	//Olinkstar must check prom.bin
	{
	...
	else if(_CREATE_LICENSE_TAG_ && GNSS_VIEWER)
	{	// In Tag version, it'll remove tag when user hold [left Shift] and 
		// [left Alt] beftore push download button.
		if(isCheat)
		{	
			m_DownloadMode = (isRomMode) ? InternalLoaderV6GpsDelTag : InternalLoaderV6GnssDelTag;
		}
		else
		{
			m_DownloadMode = (isRomMode) ? InternalLoaderV6GpsAddTag : InternalLoaderV6GnssAddTag;
		}
	}
	else if(_CREATE_LICENSE_TAG_ && GPS_VIEWER)
	{
		m_DownloadMode = (isCheat) ? InternalLoaderV6GpsDelTag : InternalLoaderV6GpsAddTag;
	}
	else if(GPS_VIEWER)
	{
		m_DownloadMode = InternalLoaderV6Gps;
	}
	else if(GNSS_VIEWER || GG12A)
	{
		m_DownloadMode = (isRomMode) ? InternalLoaderV6Gps : InternalLoaderV6Gnss;
	}
	else
	{
		ASSERT(FALSE);
...
//	char buff[128]; 
	...
	UTC_convert_utc_to_gps_time(&utc,wn,tow);
...
//void CGPSDlg::UTC_convert_utc_to_gps_time( const UTC_TIME_T *utc_time_p, S16 *wn_p, D64 *tow_p )
void UTC_convert_utc_to_gps_time( const UTC_TIME_T *utc_time_p, S16 *wn_p, D64 *tow_p )
{
	...
	total_int_sec +=  DEFAULT_LEAP_SECS; // because GPS-UTC = +14 seconds    
...
void CGPSDlg::ShowBinaryOutput(unsigned char *buff,int len)
{
	U16 wn;
	U32 tow;
	D64 ftow;
//	U32 lat,lon,alt,alt_t,mm;
	U32 alt, alt_t;
	D64 flat,flon;
	U32 m_gdop,m_pdop,m_hdop,m_vdop,m_tdop;
	S32 vx,vy,vz;
	D64 fx,fy,fz,fv;
	char m_buff[512];
	int msg_len;
	S32 ecefx,ecefy,ecefz;

	if(buff[5]==0)
	{
		m_gpggaMsgCopy.GPSQualityIndicator = '0';
		m_gpgsaMsgCopy.Mode = 1;
		m_gprmcMsgCopy.ModeIndicator = 'V';
	}
	else if(buff[5] ==1)
	{
		m_gpggaMsgCopy.GPSQualityIndicator = '1';
		m_gpgsaMsgCopy.Mode = 2;
		m_gprmcMsgCopy.ModeIndicator = 'A';
	}
	else if(buff[5] == 2)
	{
		m_gpggaMsgCopy.GPSQualityIndicator = '1';
		m_gpgsaMsgCopy.Mode = 3;
		m_gprmcMsgCopy.ModeIndicator = 'A';
	}
	else if(buff[5] == 3)
	{
		m_gpggaMsgCopy.GPSQualityIndicator = '2';
		m_gpgsaMsgCopy.Mode = 3;
		m_gprmcMsgCopy.ModeIndicator = 'A';
	}

	m_gpggaMsgCopy.NumsOfSatellites = buff[6];

	wn = buff[7] << 8 | buff[8];
	tow = buff[9]<<24 | buff[10]<<16 | buff[11]<<8 | buff[12];
	ftow = (double)tow/100;
	...
	//data_p->sec = i_sec*0.001f;

	m_gpggaMsgCopy.Hour = utc.hour;
	m_gpggaMsgCopy.Min = utc.minute;
	m_gpggaMsgCopy.Sec = i_sec*0.001f;

	m_gprmcMsgCopy.Year = utc.year;
	m_gprmcMsgCopy.Month = utc.month;
	m_gprmcMsgCopy.Day = utc.day;
	m_gprmcMsgCopy.Hour = utc.hour;
	m_gprmcMsgCopy.Min = utc.minute;
	m_gprmcMsgCopy.Sec = i_sec*0.001f;

	double lat = buff[13]<<24 | buff[14]<<16 | buff[15]<<8 | buff[16];
	double lon = buff[17]<<24 | buff[18]<<16 | buff[19]<<8 | buff[20];
	...
	m_gpggaMsgCopy.Latitude = flat;
	m_gpggaMsgCopy.Latitude_N_S = flat >= 0 ? 'N' : 'S';
	m_gpggaMsgCopy.Longitude = flon;
	m_gpggaMsgCopy.Longitude_E_W = flon >= 0 ? 'E' : 'W';

	m_gprmcMsgCopy.Latitude = flat;
	m_gprmcMsgCopy.Latitude_N_S = flat >= 0 ? 'N' : 'S';
	m_gprmcMsgCopy.Longitude = flon;
	m_gprmcMsgCopy.Longitude_E_W = flon >= 0 ? 'E' : 'W';

	alt_t =buff[21]<<24 | buff[22]<<16 | buff[23]<<8 | buff[24];

	alt = buff[25]<<24 | buff[26]<<16 | buff[27]<<8 | buff[28];
	m_gpggaMsgCopy.Altitude = (F32)alt;
	m_gpggaMsgCopy.Altitude /= 100.0F;

	m_gdop = buff[29]<<8 | buff[30];
	m_pdop = buff[31]<<8 | buff[32];
	m_hdop = buff[33]<<8 | buff[34];
	m_vdop = buff[35]<<8 | buff[36];
	m_tdop = buff[37]<<8 | buff[38];

	m_gpgsaMsgCopy.PDOP = (F32)m_pdop/100;
	m_gpggaMsgCopy.HDOP = (F32)m_hdop/100;
	m_gpgsaMsgCopy.HDOP = (F32)m_hdop/100;
	m_gpgsaMsgCopy.VDOP = (F32)m_vdop/100;

	ecefx = buff[39]<<24 | buff[40]<<16 | buff[41]<<8 | buff[42];
	ecefy = buff[43]<<24 | buff[44]<<16 | buff[45]<<8 | buff[46];
	ecefz = buff[47]<<24 | buff[48]<<16 | buff[49]<<8 | buff[50];

	vx = buff[51]<<24 | buff[52]<<16 | buff[53]<<8 | buff[54];
	vy = buff[55]<<24 | buff[56]<<16 | buff[57]<<8 | buff[58];
	vz = buff[59]<<24 | buff[60]<<16 | buff[61]<<8 | buff[62];

	fx = vx / 100;
	fy = vy / 100;
	fz = vz / 100;

	fv = sqrt(fx*fx+fy*fy+fz*fz);

	m_gprmcMsgCopy.SpeedKnots = (F32)(fv / 1.85567F);

	msg_len = sprintf_s(m_buff,"$fix mode=%d",buff[5]);
	add2message(m_buff,len);
	msg_len = sprintf_s(m_buff,"$number if sv in fix=%d",m_gpggaMsgCopy.NumsOfSatellites);
	add2message(m_buff,len);	
	...
	add2message(m_buff,len);
	msg_len = sprintf_s(m_buff,"$sea level altitude=%.2f",m_gpggaMsgCopy.Altitude);
	add2message(m_buff,len);

	msg_len = sprintf_s(m_buff,"$gdop=%.2f",(F32)m_gdop/100);
	add2message(m_buff,len);
	msg_len = sprintf_s(m_buff,"$pdop=%.2f",(F32)m_pdop/100);
	add2message(m_buff,len);
	msg_len = sprintf_s(m_buff,"$hdop=%.2f",(F32)m_hdop/100);
	add2message(m_buff,len);
	msg_len = sprintf_s(m_buff,"$tdop=%.2f",(F32)m_tdop/100);
	add2message(m_buff,len);
...
LRESULT CGPSDlg::OnMyDeviceChange(WPARAM wParam, LPARAM lParam)
{
	if( DBT_DEVICEARRIVAL == wParam || DBT_DEVICEREMOVECOMPLETE == wParam ) 
	{
		...
		case DBT_DEVTYP_PORT:
			...
			Close_Open_Port(wParam,pDevPort->dbcp_name);
			break;
...
{	
	const UiLocationEntry UiTable[] = 
	{
		...
		TRUE, 0, IDC_EARTH_T, {301, 334, 100, 19},
		TRUE, 0, IDC_SCATTER_SNR_T, {656, 334, 100, 19},
		...
		TRUE, 0, IDC_GNSS_BAR, {301, 252, 343, 75},
