...
NmeaType NMEA::MessageType(LPCSTR pt, int len)
{
	...
	if(MSG_Unknown==returnType || MSG_REBOOT==returnType)
	{
		return returnType;
	}
...
///*
void NMEA::ShowGNGSAmsg(GPGSA& rgpgsa, GPGSA& rglgsa, GPGSA& rbdgsa, LPCSTR pt, int len)
{
	if(firstGsaIn == false)
	{
		firstGsaIn = true;
...
/*
void NMEA::ShowGPGSVmsg(GPGSV& rgpgsv, LPCSTR pt, int len)
{
	firstGsaIn = false;
	GPGSV tmpGsv = { 0 };
	...
	for(int i=0; i<4; ++i)
	{
		if(0 == rgpgsv.sates[i].SatelliteID)
		{
			continue;
		}

		satellites_gps[gpgsv_counter].SatelliteID = rgpgsv.sates[i].SatelliteID;
		satellites_gps[gpgsv_counter].Elevation   = rgpgsv.sates[i].Elevation;
		satellites_gps[gpgsv_counter].Azimuth     = rgpgsv.sates[i].Azimuth;
		satellites_gps[gpgsv_counter].SNR         = rgpgsv.sates[i].SNR;   
		gpgsv_counter++;
	...
	if(rgpgsv.NumOfMessage == rgpgsv.SequenceNum)
	{
		gpgsv_counter = 0;
	}
}
...
void NMEA::ShowGPGSVmsg2(GPGSV& rgpgsv, GPGSV& rglgsv, GPGSV& rbdgsv, LPCSTR pt, int len)
{
	firstGsaIn = false;
	GPGSV tmpGsv = { 0 };
	GSVProc(tmpGsv, pt, len);
