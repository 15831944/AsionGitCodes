...
UINT timing_thread(LPVOID param)
{
	...
	/*
	CGPSDlg::gpsDlg->CloseOpenUart();
	CGPSDlg::gpsDlg->serial->ResetPort(buad);
	CGPSDlg::gpsDlg->m_BaudRateCombo.SetCurSel(buad);
	*/
#endif

	CGPSDlg::gpsDlg->m_ttffCount = 0;
	CGPSDlg::gpsDlg->m_initTtff = false;
	//sprintf_s(text, sizeof(text), "0");
	//CGPSDlg::gpsDlg->m_ttff.SetWindowText(text);
