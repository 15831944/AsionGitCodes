...
void CPic_Scatter::Create_scatterplot(CDC *dc)
{
	...
	if(coor==0)
	{
		...
		{				
			...
			str.Format("%d", g_scatterData.m_enuScale * (i-2));
			...
			str.Format("%d", g_scatterData.m_enuScale * j);
	...
	{	
		...
		vector<ScatterPoint>::iterator llaPointSetIter;
		for(llaPointSetIter = g_scatterData.LLAPointSet.begin(); 
			llaPointSetIter != g_scatterData.LLAPointSet.end(); ++llaPointSetIter)
		{
