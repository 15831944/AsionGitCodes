...
void CGPSDlg::Download()
{
	...
	U16 crcCode = 0;
	if(m_DownloadMode != EnternalLoader && CUSTOMER_DOWNLOAD && CUSTOMER_ID==0x4F4C)
	{
	...
	switch(m_DownloadMode)
	{
		...
		{	//Using command to initial loader.
			...
			else if(m_DownloadMode==CustomerDownload)
			{
				msg[0] = 0x64;
				msg[1] = 0x7F;
				msg[2] = HIBYTE(CUSTOMER_ID);
				msg[3] = LOBYTE(CUSTOMER_ID);
				msg[4] = 0;
...
//		Sleep(5000);
	...
	m_testDuration = 60;
	m_rebootCount = 0;
	SetTimer(TEST_KERNEL_TIMER, 1000, NULL);
#endif
}
