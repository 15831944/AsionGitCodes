...
bool CSerial::Open2(LPCSTR comPort, int baudRate)
{
	return OpenByBaudrate(comPort, baudRate);
}
...
DWORD CSerial::GetBinary(void *buffer, DWORD bufferSize)
{	
...
{	
	...
	{ 
		DWORD nBytesRead = ReadData(bufferIter, 1);
 		//if(m_logStatus && nBytesRead)
		{
		//	CString dbg;
		//	dbg.Format("%02x ", bufferIter[0]);
		...
		if(totalSize > 0)
		{
			U08 sizeHigh = (totalSize>3) ? *((U08*)buffer + 2) : 0;
			U08 sizeLow = (totalSize>3) ? *((U08*)buffer + 3) : 0;
			switch(CheckBufferContent(*bufferIter, *(bufferIter-1), *((U08*)buffer), sizeHigh, sizeLow, totalSize))
			{
			case BinaryMessageStart:
				bufferIter = (U08*)buffer;
				bufferIter[0] = BINARY_HD1;
				bufferIter[1] = BINARY_HD2;
				bufferIter += 2;
				totalSize = 2;
				continue;
			case BinaryMessageEnd:
				*(bufferIter + 1) = 0;
				return ++totalSize;
			case MessageEnd:
				return totalSize;
